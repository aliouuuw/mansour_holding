  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-primary-950">Mansour Motors</h1>
          <p className="mt-1 text-sm text-muted">Tableau de bord du concessionnaire</p>
        </div>
        <div className="flex gap-3">
          <Link
            to="/dashboard/motors/inventory"
            className="inline-flex items-center gap-2 rounded-lg border border-border bg-white px-4 py-2 text-sm font-medium text-primary-900 shadow-sm hover:bg-surface-dim transition-colors"
          >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpis.map((kpi) => (
          <div key={kpi.label} className="rounded-xl border border-border bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-muted">{kpi.total}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        {/* Recent Activity */}
        <div className="lg:col-span-3 rounded-xl border border-border bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 className="font-semibold text-primary-950">Activité récente</h2>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
            <h2 className="font-semibold text-primary-950">Pipeline des ventes</h2>
            <div className="mt-4 space-y-3">
              {[
                { label: 'Prospects', count: deals.filter((d) => d.status === 'prospect').length, color: 'bg-blue-500' },
                { label: 'Négociation', count: deals.filter((d) => d.status === 'negotiation').length, color: 'bg-amber-500' },
                { label: 'Essai', count: deals.filter((d) => d.status === 'test-drive').length, color: 'bg-purple-500' },
                { label: 'Conclu', count: deals.filter((d) => d.status === 'closed-won').length, color: 'bg-emerald-500' },
                { label: 'Perdu', count: deals.filter((d) => d.status === 'closed-lost').length, color: 'bg-red-500' },
              ].map((stage) => (
                <div key={stage.label} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={cn('h-2.5 w-2.5 rounded-full', stage.color)} />
                    <span className="text-sm text-primary-900">{stage.label}</span>
                  </div>
                  <span className="text-sm font-semibold text-primary-950">{stage.count}</span>
                </div>
              ))}
            </div>
            <Link
          ...
          <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
            <h2 className="font-semibold text-primary-950">Véhicules populaires</h2>
            <div className="mt-4 space-y-3">
              {vehicles.slice(0, 3).map((v) => (
                <Link
                  ...
                  className="flex items-center gap-3 rounded-lg p-2 -mx-2 hover:bg-surface-dim transition-colors"
                >
                  <img
                    src={v.image}
                    alt={`${v.make} ${v.model}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted">{v.year} · {formatPrice(v.price)}</p>
                  </div>
                </Link>
              ))}
            </div>
